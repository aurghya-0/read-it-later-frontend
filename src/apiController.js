...
export const getAllFeeds = async (req, res) => {
  try {
    ...
    res.status(200).json({ feeds, selectedFeed: null, articles: [] });
  } catch (err) {
    console.error(err);
  }
};
...
export const getAllArticlesFromFeed = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ feeds, selectedFeed: feed, articles });
  } catch (err) {
    console.error(err);
  }
};
...
export const getArticlesByCategory = async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ message: "Error retrieving articles by category" });
  }
};
